...
namespace Common.Logging.Log4Net
{
    ...
    public class CommonLoggingAppender : AppenderSkeleton
    {
        ...
        protected static Level GetClosestLevel(Level currentLevel)
        {
            ...
            if (currentLevel >= Level.Fatal) return Level.Fatal;
            if (currentLevel >= Level.Error) return Level.Error;
            if (currentLevel >= Level.Warn) return Level.Warn;
            if (currentLevel >= Level.Info) return Level.Info;
            if (currentLevel >= Level.Debug) return Level.Debug;
            if (currentLevel >= Level.Trace) return Level.Trace;
